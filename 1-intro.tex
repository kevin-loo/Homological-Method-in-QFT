...
% Text Node
\draw (21.2,98.64) node [anchor=north west][inner sep=0.75pt]   [align=left] {{\huge $\lb \ \ \ \ \ \ \ \ \ \ \rb$}};
% Text Node
\draw (304.9,98.64) node [anchor=north west][inner sep=0.75pt]   [align=left] {{\huge $\lb \ \ \ \ \ \ \ \ \ \ \rb$}};
% Text Node
...
\begin{eg}[Chiral QFT] $\on{dim }X=2$ (a plane). 
\bea
\tikzset{every picture/.style={line width=0.75pt}} %set default line width to 0.75pt    
\begin{tikzpicture}[x=0.75pt,y=0.75pt,yscale=-1,xscale=1]
%uncomment if require: \path (0,300); %set diagram left start at 0, and has height of 300
...
\paragraph{BV approach.}
    ...
    \bea 
    \on{PV}^\bd(M) =\bigoplus_k \on{PV}^k(M)\coloneqq \bigoplus_k \Gamma \lb M,\ \asym^k TM\rb.
     \eea
