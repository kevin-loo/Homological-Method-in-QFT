...
$O(\cE)$ (or certain homology). For example, 
...
\bea
\operatorname{Obs}(U)=\text{functions on }  \cE(U).
\eea
...
% Text Node
\draw (434.14,108.24) node   [align=left] {{\Large ( \ \ \ \ \ \ \ )}};
% Text Node
\draw (495.55,108.24) node   [align=left] {{\Large ( \ \ \ \ \ \ \ )}};
% Text Node
\draw (469.6,196.37) node   [align=left] {{\Large ( \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ )}};
% Text Node
...
\paragraph{BV approach.}
    ...
    PV^{n} \ar[r, "\Delta"]
    & PV^{n-1} \ar[r, "\Delta"]
    & \cdots \ar[r, "\Delta"] & PV^{0} \ar[ur, "\int_{\operatorname{BV}}"] & 
    \end{tikzcd}
    \eea
where
...
\bea \int_{\operatorname{BV}}: \operatorname{PV}^0 \to \bR,\qquad 
