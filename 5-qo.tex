...
One way out of this problem is to use regularization to define an effective QME. We defined the Poisson kernel $K_0=K_r+QP_r$, where $K_r$ is smooth, leading to a well-defined BV operator $\Delta_r$ on $\cO(\cE)$. For each $r$, we have constructed $I[r]\in \cO(\cE)$ solving the effective QME:
...
To cancel the divergence, one can find the \emph{counter-terms}, $I^\eps\in \hbar \cO_{\text{loc}}(\cE)\llb \hbar\rrb$, which are $\vep$-dependent and singular as $\vep\to 0$ such that
...
\eea
In other words, the family $\lcb I[L]^{\text{naive}}\rcb_{L>0}$ satisfies the homotopy RG (HRG) flow.
\bea
\tikzset{every picture/.style={line width=0.75pt}} %set default line width to 0.75pt
\begin{tikzpicture}[x=0.75pt,y=0.75pt,yscale=-1,xscale=1]
%uncomment if require: \path (0,300); %set diagram left start at 0, and has height of 300
...
% Text Node
\draw (40.5,108.07) node [anchor=north west][inner sep=0.75pt]    {$I[L_{1}]^{\text{naive}}$};
% Text Node
\draw (152.5,108.07) node [anchor=north west][inner sep=0.75pt]    {$I[L_{2}]^{\text{naive}}$};
% Text Node
\draw (110,106.57) node [anchor=north west][inner sep=0.75pt]    {$\overset{\text{HRG}}{\leadsto}$};
\end{tikzpicture}
\eea
However, $\lcb I[L]^{\text{naive}}\rcb_{L>0}$ may NOT satisfy the QME!
